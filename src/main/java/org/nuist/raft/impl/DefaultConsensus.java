...
public class DefaultConsensus implements Consensus {
    ...
    public RvoteResult requestVote(RvoteParam param) {
        try {
            ...
            if ((StringUtil.isNullOrEmpty(node.getVotedFor()) || node.getVotedFor().equals(param.getCandidateId()))) {
                ...
                LOGGER.info("param{}",node.getVotedFor());
                node.setVotedFor(param.serverId);
                LOGGER.info("投票{}",node.getVotedFor());
                // 返回成功
     ...
     */
    @Override
    public AentryResult appendEntries(AentryParam param) throws Exception{
        LOGGER.info("心跳前{}",node.getVotedFor());
        ...
        try {
            ...
            LOGGER.info("心跳后{}",node.getVotedFor());
            appendLock.unlock();
        }
